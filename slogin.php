...
<center>
  ...
  <fieldset style="height: 640px;width:80%"><legend align="center">Welcome to Login</legend>
  <h3>USERNAME <font color="red">*</font></h3><br>
  <input type="text" style="height: 50px;width:50%" name="name"  pattern="\w+"
        title="Username should only contain  " ><br><br>
  ...
  <h3 >PASSWORD <font color="red">*</font></h3><br>
   <input type="password" style="height: 50px;width:50%" name="password"  title="Password must include at least 1 letter, 1 number, and 1 special character, and be 8–16 characters long."
     pattern="^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,16}$"  /><br><br>
  
   <input type="submit" style="height: 50px;width:50%" name="submit" value="LOGIN" class="btn">
  <h5>If  You Dont Have an Account Please <a href="registration.php">Register Here</a></h5>
  </fieldset>
</form>
</center>
</body>
...
if(isset($_POST['submit']))
{
  ...
  $res=mysqli_query($conn,"SELECT * FROM register WHERE  user_id='$user_id' && name='$name'");
